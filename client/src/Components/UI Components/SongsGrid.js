...
export default function SongsGrid({ items }) {
  ...
  return (
    <Grid>
      {items &&
        items.items.map((el) => (
          <SongContainer key={el.id}>
            <SpotifyLink>
              <SongImage img={el.album.images[1].url}></SongImage>
              <a
            ...
            <span id="duration">{secondsToMinutes(el.duration_ms / 1000)}</span>
          </SongContainer>
        ))}
    </Grid>
  );
